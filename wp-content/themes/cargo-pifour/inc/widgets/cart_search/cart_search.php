...
class cargo_pifour_Widget_Cart_Search extends WP_Widget
{
    protected $settings;
    ...
    {
        extract(array(
            'id_base'        => 'cargo_pifour_widget_cart_search',
            'name'           => esc_html__('Cart & Search', 'cargo-pifour'),
            'widget_options' => array(
                'description' => esc_html__('Display the user\'s Cart and Search form in the sidebar.', 'cargo-pifour')
            )
        ));
        ...
        $this->settings = array(
            'title'                    => array(
                'type'  => 'text',
                'std'   => esc_html__('Cart & Search', 'cargo-pifour'),
                'label' => esc_html__('Title', 'cargo-pifour')
            ),
            'show_cart'                => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => esc_html__('Show Cart', 'cargo-pifour'),
            ),
            'show_cart_contents_count' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => esc_html__('Show Count', 'cargo-pifour'),
            ),
            'show_cart_subtotal'       => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => esc_html__('Show Subtotal', 'cargo-pifour'),
            ),
            'show_search'              => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => esc_html__('Show Search', 'cargo-pifour'),
            ),
            'search_bar_style'         => array(
                'type'    => 'select',
                'std'     => 'dropdown',
                'options' => array(
                    'dropdown' => esc_html__('Dropdown', 'cargo-pifour'),
                    'popup'    => esc_html__('Popup', 'cargo-pifour')
                ),
                'label'   => esc_html__('Search Bar Style', 'cargo-pifour'),
            ),
            'add_class'                => array(
                'type'  => 'text',
                'std'   => '',
                'label' => esc_html__('Add Class', 'cargo-pifour')
            )
        );
    ...
    {
        wp_enqueue_script('wpcargo-pifour-widget_cart_search_scripts', get_template_directory_uri() . '/inc/widgets/cart_search/cart_search.js', array('jquery'), '1.0.1', true);
        wp_enqueue_style('wpcargo-pifour-widget_cart_search_scripts', get_template_directory_uri() . '/inc/widgets/cart_search/cart_search.css');
    }
    ...
    public function woocommerce_header_add_to_cart_content($fragments)
    {
        ...
        foreach ($cart as $key => $item) {
            ...
            $data = $woocommerce->cart->get_item_data($item);
            ...
            $remove_link = $woocommerce->cart->get_remove_url($key);
            $id = $_product->get_id();
...
function cargo_pifour_register_cart_search_widget()
{
    register_widget('cargo_pifour_Widget_Cart_Search');
}

add_action('widgets_init', 'cargo_pifour_register_cart_search_widget');
