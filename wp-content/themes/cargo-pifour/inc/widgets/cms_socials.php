<?php
add_action( 'widgets_init', create_function( '', "register_widget( 'Cms_Socials_Widget' );" ) );
...
class Cms_Socials_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            ...
            esc_html__('Cms Socials', 'cargo-pifour'), // Name
            array('description' => esc_html__('Socials Widget', 'cargo-pifour')) // Args
        );
    }
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Be Social', 'cargo-pifour' ) : $instance['title'], $instance, $this->id_base);
        }
        ...
        for($i=1; $i<=10; $i++){
            ...
            if(!empty($icon_class_i) && !empty($link_i))
                echo '<li><a target="_blank" href="'.esc_url($link_i).'"><i class="'.esc_attr($icon_class_i).'"></i></a></li>';
        }
    ...
    function form( $instance ) {
         ...
         ?>
         <p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e( 'Title:', 'cargo-pifour' ); ?></label>
         ...
         ?>
             <p>
             <label for="<?php echo esc_attr($this->get_field_id('icon_class_'.$i)); ?>"><?php esc_html_e( 'Icon class', 'cargo-pifour' ); ?> <?php echo esc_attr($i);?></label>
             ...
             <p>
             <label for="<?php echo esc_attr($this->get_field_id('link_'.$i)); ?>"><?php esc_html_e( 'Link', 'cargo-pifour' ); ?> <?php echo esc_attr($i);?></label>
         ...
         ?>   
         <p><label for="<?php echo esc_attr($this->get_field_id('extra_class')); ?>"> <?php esc_html_e( 'Extra Class:', 'cargo-pifour' ); ?></label>
