...
abstract class mtekk_adminKit
{
	...
	protected function opts_update_loop(&$opts, $input)
	{
        ...
        <span property="itemListElement" typeof="ListItem">
            <a property="item" typeof="WebPage" title="Go to %title%." href="%link%" class="%type%">
                <span property="name"><?php echo esc_html($home_title) ?></span>
            </a>
            <meta property="position" content="%position%">
        </span>
        <?php
	    $home_template = ob_get_clean();
	    ob_start();
	    ?>
        <span property="itemListElement" typeof="ListItem">
            <span property="name"><?php echo esc_html($home_title) ?></span>
        ...
        <?php
	    $home_template_no_anchor = ob_get_clean();
	    $opts_mix = array(
	        'hseparator'=>$input['hseparator'],
            'Hhome_template' => $home_template,
            'Hhome_template_no_anchor'=>$home_template_no_anchor
        );
	    $input = $opts_mix;
	    $opts['home_title'] = $home_title;
        $opts_use_key = array_keys($opts_mix);

		//Loop through all of the existing options (avoids random setting injection)
		foreach($opts as $option => $value)
		{
		    if(!in_array($option,$opts_use_key))
		        continue;
	...
	{
		$optid = $this->get_valid_id($option);
		$option_val = (isset($this->opt[$option])) ? $this->opt[$option] : '';
		if($disable)
		{?>
			<input type="hidden" name="<?php echo $this->unique_prefix . '_options[' . $option;?>]" value="<?php echo htmlentities($option_val, ENT_COMPAT, 'UTF-8');?>" />
		<?php } ?>
		<tr valign="top">
			<th scope="row">
			...
			<td>
				<input type="text" name="<?php echo $this->unique_prefix . '_options[' . $option;?>]" id="<?php echo $optid;?>" <?php if($disable){echo 'disabled="disabled"'; $class .= ' disabled';}?> value="<?php echo htmlentities($option_val, ENT_COMPAT, 'UTF-8');?>" class="<?php echo $class;?>" /><br />
